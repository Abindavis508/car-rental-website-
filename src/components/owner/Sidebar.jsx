  ...
  return (
    ...
    max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
      ...
      <div className="group relative">
        <label htmlFor="image">
          <img src={image ? URL.createObjectURL(image) : user?.image || "https://www.pexels.com/photo/stunning-golden-eagle-in-natural-lighting-35802444/"}
            alt="" className="h-9 md:h-14 w-9 md:w-14 
            rounded-full mx-auto" />
          <input type="file" id='image' accept="image/*" hidden onChange={e =>
            setImage(e.target.files[0])} />

          <div className="absolute hidden top-0 left-0 bottom-0 right-0
      ...
      <div className="w-full">
        {ownerMenuLinks.map((link,index)=>(
          ...
          ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-600'}`}>
            ...
            alt="car_Icon" className="" />
            <span className="max-md:hidden">{link.name}</span>
            <div className={`${link.path === location.pathname && 'bg-primary'}w-1.5 h-8 rounded-1 absolute`}></div>
          </NavLink>
        ))}
        </div>
    </div>
